...
import {
    EventPayloadDescriptor, zodSchemaEventPayloadDescriptor,
...
describe('EVENT PAYLOAD DESCRIPTOR', function() {
    ...
    describe('ZOD', function() {
        ...
        it('should parse eventPayload', function() {
            ...
            const eventPayload: EventPayloadDescriptor = zodSchemaEventPayloadDescriptor.parse(data.eventPayloadDescriptors[0]) as EventPayloadDescriptor;
        ...
        it('should parse eventPayload w/o objectType', function() {
            // console.log(data.intervals[0]);
            const eventPayload: EventPayloadDescriptor = zodSchemaEventPayloadDescriptor.parse(data.eventPayloadDescriptors[1]) as EventPayloadDescriptor;
            // console.log(eventPayload);
            assert.deepEqual(eventPayload, {
                // objectType: 'EVENT_PAYLOAD_DESCRIPTOR',
                payloadType: 'PRICE',
        ...
        it('should parse eventPayload w/o units', function() {
            // console.log(data.intervals[0]);
            const eventPayload: EventPayloadDescriptor = zodSchemaEventPayloadDescriptor.parse(data.eventPayloadDescriptors[2]) as EventPayloadDescriptor;
            // console.log(eventPayload);
            assert.deepEqual(eventPayload, {
                objectType: 'EVENT_PAYLOAD_DESCRIPTOR',
                payloadType: 'PRICE',
        ...
        it('should parse eventPayload w/o currency', function() {
            // console.log(data.intervals[0]);
            const eventPayload: EventPayloadDescriptor = zodSchemaEventPayloadDescriptor.parse(data.eventPayloadDescriptors[3]) as EventPayloadDescriptor;
            // console.log(eventPayload);
            assert.deepEqual(eventPayload, {
                objectType: 'EVENT_PAYLOAD_DESCRIPTOR',
                payloadType: 'PRICE',
        ...
        it('should parse eventPayload w/o objectType, units, currency', function() {
            // console.log(data.intervals[0]);
            const eventPayload: EventPayloadDescriptor = zodSchemaEventPayloadDescriptor.parse(data.eventPayloadDescriptors[4]) as EventPayloadDescriptor;
            // console.log(eventPayload);
            assert.deepEqual(eventPayload, {
                // objectType: 'EVENT_PAYLOAD_DESCRIPTOR',
                payloadType: 'PRICE',
        ...
        it('should parse eventPayload w/ extra data not seen', function() {
            // console.log(data.intervals[0]);
            const eventPayload: EventPayloadDescriptor = zodSchemaEventPayloadDescriptor.parse(data.eventPayloadDescriptors[5]) as EventPayloadDescriptor;
            // console.log(eventPayload);
            assert.deepEqual(eventPayload, {
                objectType: 'EVENT_PAYLOAD_DESCRIPTOR',
                payloadType: 'EXTRA',
        ...
        it('should parse eventPayload w/ extra data seen w/ passthrough', function() {
            // console.log(data.intervals[0]);
            const eventPayload: EventPayloadDescriptor = zodSchemaEventPayloadDescriptor.passthrough().parse(data.eventPayloadDescriptors[5]) as EventPayloadDescriptor;
            // console.log(eventPayload);
            assert.deepEqual(eventPayload, {
                objectType: 'EVENT_PAYLOAD_DESCRIPTOR',
                payloadType: 'EXTRA',
        ...
        it('should fail to parse eventPayload w/ extra data w/ strict', function() {
            ...
            let didFail = false;
            try {
                const eventPayload: EventPayloadDescriptor = zodSchemaEventPayloadDescriptor.strict().parse(data.eventPayloadDescriptors[5]) as EventPayloadDescriptor;
            } catch (err: any) {
                didFail = true;
        ...
        it('should fail to parse eventPayload w/ short payloadType', function() {
            ...
            let didFail = false;
            try {
                const eventPayload: EventPayloadDescriptor = zodSchemaEventPayloadDescriptor.strict().parse(data.BADeventPayloadDescriptors[0]) as EventPayloadDescriptor;
            } catch (err: any) {
                didFail = true;
                // console.log(err.issues);
                assert.deepEqual(err.issues, [
        ...
        it('should fail to parse eventPayload w/ extra-long payloadType', function() {
            ...
            let didFail = false;
            try {
                const eventPayload: EventPayloadDescriptor = zodSchemaEventPayloadDescriptor.strict().parse(data.BADeventPayloadDescriptors[1]) as EventPayloadDescriptor;
            } catch (err: any) {
                didFail = true;
                // console.log(err.issues);
                assert.deepEqual(err.issues, [
