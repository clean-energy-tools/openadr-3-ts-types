...
import {
    IntervalPeriod, zodSchemaIntervalPeriod,
    joiIntervalPeriod
...
describe('INTERVAL PERIOD', function() {
    ...
    describe('ZOD', function() {
        ...
        it('should parse intervalPeriod', function() {
            ...
            const intervalPeriod: IntervalPeriod = zodSchemaIntervalPeriod.parse(data.intervalPeriod[0]) as IntervalPeriod;
        ...
        it('should parse intervalPeriod w/ randomizeStart', function() {
            // console.log(data.intervals[0]);
            const intervalPeriod: IntervalPeriod = zodSchemaIntervalPeriod.parse(data.intervalPeriod[1]) as IntervalPeriod;
            // console.log(intervalPeriod);
            assert.deepEqual(intervalPeriod, {
                start: '2023-03-26T18:30:28.829Z',
                duration: 'PT15M',
        ...
        it('should parse intervalPeriod w/ negative durations', function() {
            // console.log(data.intervals[0]);
            const intervalPeriod: IntervalPeriod = zodSchemaIntervalPeriod.parse(data.intervalPeriod[2]) as IntervalPeriod;
            // console.log(intervalPeriod);
            assert.deepEqual(intervalPeriod, {
                start: '2023-03-26T18:30:28.829Z',
                duration: '-PT15M',
        ...
        it('should parse intervalPeriod w/ extra data and see it with passthrough', function() {
            // console.log(data.intervals[0]);
            const intervalPeriod: IntervalPeriod = zodSchemaIntervalPeriod.passthrough().parse(data.intervalPeriod[3]) as IntervalPeriod;
            // console.log(intervalPeriod);
            assert.deepEqual(intervalPeriod, {
                start: '2023-03-26T18:30:28.829Z',
                duration: '-PT15M',
        ...
        it('should fail to parse intervalPeriod w/ extra data with strict', function() {
            ...
            let didFail = false;
            try {
                const intervalPeriod: IntervalPeriod = zodSchemaIntervalPeriod.strict().parse(data.intervalPeriod[3]) as IntervalPeriod;
            } catch (err: any) {
                didFail = true;
        ...
        it('should fail to parse intervalPeriod w/ bad data', function() {
            ...
            let didFail = false;
            try {
                const intervalPeriod: IntervalPeriod = zodSchemaIntervalPeriod.parse(data.BADintervalPeriod[0]) as IntervalPeriod;
            } catch (err: any) {
                didFail = true;
                // console.log(err.issues);
                assert.deepEqual(err.issues, [
