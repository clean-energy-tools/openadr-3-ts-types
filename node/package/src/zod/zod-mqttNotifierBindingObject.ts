...
export default z.object({ "URIS": z.array(z.string().url().describe("URIs for connection to MQTT broker")), "serialization": z.literal("JSON").describe("Currently always JSON, perhaps other formats supported in future"), "authentication": z.any().superRefine((x, ctx) => {
  ...
  }).describe("Authentication method supported for connection to MQTT broker") }).describe("Details of MQTT binding for messaging protocol support");
